        ...
        <div className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              href={link.path}
              className={`text-[10px] uppercase tracking-[0.2em] hover:text-primary relative group py-2 transition-colors ${pathname === link.path ? 'text-primary' : 'text-textMain/70'
                }`}
            >
              {link.name}
              <span className={`absolute bottom-0 left-0 w-0 h-[2px] bg-primary group-hover:w-full transition-all duration-300 ${pathname === link.path ? 'w-full' : ''
                }`} />
            </Link>
          ))}
          <button
                ...
                {navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      href={link.path}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`text-2xl font-syne font-bold uppercase tracking-tighter transition-colors ${pathname === link.path ? 'text-primary' : 'text-textMain'
                        }`}
                    >
                      {link.name}
                    </Link>

...
const VisaCard: React.FC<VisaCardProps> = ({ title, time, status, index }) => {
  return (
    <div
      className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.03)] flex flex-col justify-between group hover:shadow-primary/5 transition-all duration-300 hover:-translate-y-1"
    >
      <div>
        <div className="flex justify-between items-start mb-8">
          <h3 className="text-xl font-syne font-bold text-slate-900 group-hover:text-primary pr-4 transition-colors duration-300">{title}</h3>
        ...
        <div className="flex items-center gap-4 mb-10">
          <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center group-hover:bg-primary transition-colors duration-300">
            <Clock className="text-primary group-hover:text-white transition-colors duration-300" size={24} />
          </div>
